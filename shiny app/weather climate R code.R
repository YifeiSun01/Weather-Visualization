...
plot_air_quality_2020=function(filname,extension){
options(warn=-1)
df = read.csv(filname)
city_name = str_to_title(gsub("-air-quality.csv", "", filname))
...
theme( # remove the vertical grid lines
  ...
  labs(
    title = paste(city_name,"Air Quality Index Radial 2020"),
    subtitle = " ",
    caption = " ",
    x = NULL,
    y = NULL, 
...
ggsave(file=paste(city_name," ","Air Pollution 2020.",extension,sep=""), plot=image_1, width=6, height=6)
}


## -------------------------------------------------------------------------------------------------------------

plot_weather_precipitation_return = function(filname){
df = read.csv(filname)
df$datetime=as.Date(df$datetime, origin = "2022-01-01")
weather_conditions = c("Clear","Partially cloudy","Overcast","Rain","Snow","Ice",
                       "Freezing Drizzle/Freezing Rain")
...
geom_segment(aes(
 ...
 geom_text(
    data = data.frame(x = x_cen-0.013+rep(0,10),
                      y = y_cen+0.0315+to_vec(for(i in 0:time) i*step), label = c("-40","-22","-4","14","32","50","68","86","104","122")),
    aes(x, y, label = label),
    hjust = 0.5, vjust = 0.1, angle = 0, size = 10/.pt,
    color = "black",
    inherit.aes = FALSE,alpha=0.8
 ...
 geom_text(
    data = data.frame(x = x_cen+0.014,
                      y = y_cen+0.0315+time*step, label = c("\u00B0F")),
    aes(x, y, label = label),
    hjust = 0.5, vjust = 0.1, angle = 0, size = 10/.pt,
    color = "black",
    inherit.aes = FALSE,alpha=0.8
...
geom_segment(aes(
 ...
 geom_text(
    data = data.frame(x = 0.04,
                      y = 0.05,
                      label = c("Daily temperatures are shown as the color of the bars, \n the length of the bar represents the range of the temperature on that day, \n the opaque part of the bar shows the feels-like temperature on that day.")),
    aes(x, y, label = label),
    hjust = 0, angle = 0, size = 10/.pt,
    color = "black",
    inherit.aes = FALSE,alpha=1
...
geom_text(data = data.frame(x = df$datetime[seq(1,365,91)[1:4]],
          ...
          aes(x,y, label=label), color = color_3, alpha=num_opacity,size=num_size)+
geom_text(data = data.frame(x = df$datetime[seq(1,365,91)[1:4]],
                      y = rep(list_time_ratio[25]+num_offset+0.017,4), label = rep(list_time_str[25],4)),
          aes(x,y, label=label), color = "#000000", alpha=num_opacity,size=num_size)+
...
geom_text(
  data = data.frame(x = df$datetime[1],
                    y = 0, label = city_name),
  aes(x, y, label = label),
  hjust = 0.5, vjust = 0.1, angle = 0, size = 17/.pt,
  color = "#333333",
  inherit.aes = FALSE
)+
...
plot_all_4=function(filname_1,filname_2,extension){
  filname_3 = paste("C:\\Users\\DELL\\Desktop\\学习\\2023 winter\\SI 649 Information Visualization\\group project\\data\\air pollution data",filname_2, sep = "\\")
  city_name = str_to_title(gsub(" *[0-9].*$", "", filname_1))
df <- data.frame(x = numeric(), y = numeric())
layout <- c(
  area(t = 0, l = 0, b = 16.5, r = 16.5),
  area(t = 0, l = 0, b = 9, r = 9),
  area(t = 0, l = 9.5, b = 7, r =16.5),
  area(t = 8, l = 8, b = 14, r = 14),
  area(t = 10, l = 1.5, b = 16, r = 7.5),
  area(t = 16, l = 0, b = 16.5, r = 16.5)
)
empty_1 = ggplot(df, aes(x, y)) + 
  geom_blank() +
  theme_void() +
  labs(x = NULL, y = NULL) + 
  theme(panel.border = element_blank(), 
          panel.background = element_blank())
empty_2 = ggplot(df, aes(x, y)) + 
  geom_blank() +
  theme_void() +
  labs(x = NULL, y = NULL) + 
  theme(panel.border = element_blank(), 
          panel.background = element_blank())
precipitation <- plot_weather_precipitation_return(filname_1)
daytime <- plot_daytime_return(filname_1)
weather_condition <- plot_weather_condition_return(filname_1)
air_quality <- plot_air_quality_return(filname_3)
image_2 = empty_2+precipitation+daytime+weather_condition+air_quality+empty_1+
  plot_layout(design = layout)

ggsave(file=paste(city_name," ","Weather 2022.",extension,sep=""), plot=image_2, width=17, height=17)
}
...
plot_all_3=function(filname_1,extension){
  city_name = str_to_title(gsub(" *[0-9].*$", "", filname_1))
df <- data.frame(x = numeric(), y = numeric())
layout <- c(
  area(t = 0, l = 0, b = 16.5, r = 16.5),
...
daytime <- plot_daytime_return(filname_1)
weather_condition <- plot_weather_condition_return(filname_1)
image_2 = empty_2+precipitation+daytime+weather_condition+empty_1+
  plot_layout(design = layout)
...
weather_cities = unlist(lapply(weather_files, function(x) str_to_title(gsub(" *[0-9].*$", "", x))))
air_cities = unlist(lapply(air_files, function(x)  str_to_title(gsub("-.*$", "", x))))
...
plot_weather_precipitation_return_new <- function(city){
