...
ui <- fluidPage(
  ...
  titlePanel("     Visualize Weathers of Cities Around the World"),
  navbarPage(
    ...
    tabPanel("Climates Around the World",
             sidebarLayout(
               ...
               mainPanel(
                 ...
                 leafletOutput("map_1")),
                   tabPanel("780 Cities",
                 leafletOutput("map_1_780")),
                   tabPanel("44351 Cities",
                 leafletOutput("map_1_44351"),
                 br(),
                 br(),
                 tabsetPanel(
                   tabPanel("See Population Distribution by Climate Types",
                            reactableOutput("df_101")
                            ),
                   tabPanel("See Top 100 Cities by Population",
                            reactableOutput("df_102")
                            )
                   )
                 )
                    ),
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 selectInput("plot_1_y_transform", "Choose y axis transformation for plot 1:", 
                             choices = c("Linear","Logarithmic","Absolute","Cosine"),
                             selected = "Linear"),
                 selectInput("continent_2", "Filter by Continent:", 
                             choices = c("(All)",sort(unique(df$Continent))),
                             selected = "(All)"),
                 selectInput("region_2", "Filter by Region:", 
                             choices = c("(All)",region_list_2),
                             selected = "(All)"),
                 ...
                 selectInput("plot_2_z_transform", "Choose z axis transformation for plot 2:", 
                             choices = c("Linear","Logarithmic","Absolute","Cosine"),
                             selected = "Linear"),
                 selectInput("continent_3", "Filter by Continent:", 
                             choices = c("(All)",sort(unique(df$Continent))),
                             selected = "(All)"),
                 selectInput("region_3", "Filter by Region:", 
                             choices = c("(All)",region_list_2),
                             selected = "(All)"),
                 ...
                 selectInput("map_3_feature", "Choose a feature to visualize on the map:", 
                             choices = keys(viz_cols_mapping),
                             selected = "Annual Mean Temperature (\u00B0C)"),
                 selectInput("continent_4", "Filter by Continent:", 
                             choices = c("(All)",sort(unique(df$Continent))),
                             selected = "(All)"),
                 selectInput("region_4", "Filter by Region:", 
                             choices = c("(All)",region_list_2),
                             selected = "(All)"),
